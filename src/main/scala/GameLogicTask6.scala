...
class GameLogicTask6(SpriteNumber: Int, BackTileNumber: Int, TuneNumber: Int) extends Module {
  ...
  io.spriteXPosition(3) := (240).S // volcano position
  ...
  switch(stateReg) {
    ...
    is(compute1) {
      ...
      // Bubble movement
      bubbleY := Mux(bubbleY <= 96.S, 480.S, bubbleY - 2.S)
